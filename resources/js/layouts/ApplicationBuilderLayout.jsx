import { ArrowLeft, Database, ExternalLink, LayoutDashboard, Settings, TableProperties } from 'lucide-react';
...
export default function ApplicationBuilderLayout() {
    ...
    return (
        <div>
            ...
            <header className="mt-6 flex flex-col justify-between gap-5 rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:p-6"><div className="flex min-w-0 items-center gap-4"><span className="grid size-13 shrink-0 place-items-center rounded-2xl bg-brand-50 text-brand-700 ring-1 ring-brand-100"><ApplicationIcon name={application.icon} size={24} /></span><div className="min-w-0"><div className="flex items-center gap-2"><h1 className="truncate text-2xl font-bold tracking-[-0.035em] text-slate-950">{application.name}</h1><span className={`hidden rounded-full px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider sm:inline ${application.status === 'active' ? 'bg-brand-50 text-brand-700' : 'bg-slate-100 text-slate-500'}`}>{application.status}</span></div><p className="mt-1 truncate text-sm text-slate-500">{workspace.name} · Build mode</p></div></div><div className="flex flex-wrap gap-2"><Link to={`/apps/${application.id}`} className="btn-primary shrink-0"><ExternalLink size={16} /> Open App</Link><Link to={`/workspaces/${workspace.id}`} className="btn-secondary shrink-0"><ArrowLeft size={16} /> Back</Link></div></header>
            <div className="mt-6 overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm lg:grid lg:min-h-[580px] lg:grid-cols-[220px_1fr]">
                <aside className="border-b border-slate-200 bg-slate-50/70 p-4 lg:border-b-0 lg:border-r"><p className="px-3 pb-3 pt-1 text-[10px] font-bold uppercase tracking-[0.15em] text-slate-400">Build mode</p><nav className="grid grid-cols-2 gap-1 sm:grid-cols-4 lg:grid-cols-1"><BuilderLink to={basePath} icon={LayoutDashboard} label="Overview" end /><BuilderLink to={`${basePath}/modules`} icon={TableProperties} label="Modules" /><DisabledBuilderNav icon={Database} label="Data" /><DisabledBuilderNav icon={Settings} label="Settings" /></nav><div className="mt-6 hidden rounded-xl border border-brand-100 bg-brand-50 p-3 text-xs leading-5 text-brand-800 lg:block"><span className="font-semibold">Build → Publish → Run</span><br />Published modules become usable instantly in Open App.</div></aside>
