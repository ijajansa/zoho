...
export default function ModuleDetailPage() {
    ...
    const [schemaSummary, setSchemaSummary] = useState({ schema_status: module.schema_status, schema_version: module.schema_version || 0 });
    ...
    return <div><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-brand-700">Fields</p><h2 className="mt-2 text-2xl font-bold tracking-tight text-slate-950">Define {module.name} data</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">{module.description || 'Fields define the data captured by this module and the form used to collect it.'}</p></div><Link to={formPath} className="btn-primary shrink-0"><Plus size={16} /> Open form builder</Link></div>
        <div className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Stat icon={Columns3} value={loading ? '-' : fields.length} label="Fields" /><Stat icon={TableProperties} value="0" label="Records" /><Stat icon={Columns3} value={schemaLabel(schemaSummary.schema_status)} label="Schema" small /><Stat icon={CalendarDays} value={`Version ${schemaSummary.schema_version}`} label={`Created ${formatDate(module.created_at)}`} small /></div>
        <SchemaPanel workspaceId={workspaceId} applicationId={applicationId} moduleId={moduleId} onStatusChange={setSchemaSummary} />
...
function schemaLabel(status) { return status === 'out_of_sync' ? 'Changes Pending' : status ? status.charAt(0).toUpperCase() + status.slice(1) : 'Draft'; }
